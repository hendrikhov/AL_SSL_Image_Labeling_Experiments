...
from sklearn.metrics import classification_report, precision_recall_curve,log_loss
...
from torch.utils.data import ConcatDataset, Dataset, TensorDataset,DataLoader , Subset, WeightedRandomSampler
...
def balance_classes(features, labels):
    ...
    balanced_features, balanced_labels = adasyn.fit_resample(features, labels)
...
def generate_bow_histograms(dataset, kmeans):
    ...
    return filtered_dataset, histograms
...
images_dir = './TransferFolder/tiny-imagenet-subset'
annotations_path = './TransferFolder/ImageNet_subset_labels.json'
...
full_dataset = TinyImageNetDataset(img_dir=images_dir, annotation_path=annotations_path, transform=transform)
...
labeled_data_sizes = [2000] # 500, 1000, 1500, 
...
for num_labeled in labeled_data_sizes:
    ...
    train_dataset, train_histograms = generate_bow_histograms(train_dataset, kmeans)
    val_dataset, val_histograms = generate_bow_histograms(val_dataset, kmeans)
    test_dataset, test_histograms = generate_bow_histograms(test_dataset, kmeans)
    ...
    if train_labels.ndim == 2 and train_labels.shape[1] > 1:
        ...
        train_labels = np.eye(20)[train_labels]

    # Balance BoVW histograms
    #print("Balancing BoVW histograms with ADASYN...")
    #train_histograms, train_labels_balanced = ADASYN(random_state=42).fit_resample(train_histograms, train_labels)
    ...
    #print(f"Labeled Indices: {labeled_indices[:5]}")  # First few labeled indices
    #print(f"Type of labeled_indices: {type(labeled_indices)}")
    ...
    weights_dict_xgb = class_weights
    ...
    for iteration in range(22):
        ...
        print("Length labeled_indices:", len(labeled_indices))
        print("Length valid_labeled_indices before vaidation:", len(valid_labeled_indices))
        valid_labeled_indices = [idx for idx in labeled_indices if idx < len(train_labels_balanced)]
        print("Length valid_labeled_indices after validation:", len(valid_labeled_indices))
        ...
        if missing_classes:
            ...
            for missing_class in missing_classes:
                ...
                synthetic_label[missing_class] = 1 
        ...
        print(f"max(unlabeled_indices): {max(unlabeled_indices)}")
        print(f"len(train_dataset): {len(train_dataset)}")
        #unlabeled_loader = DataLoader(Subset(train_dataset, unlabeled_indices), batch_size=batch_size, shuffle=False)
        valid_unlabeled_indices = [idx for idx in unlabeled_indices if idx < len(train_dataset)]
        #print(f"Valid unlabeled_indices: {valid_unlabeled_indices}")
        print(f"len(valid_unlabeled_indices): {len(valid_unlabeled_indices)}")
        unlabeled_loader = DataLoader(Subset(train_dataset, valid_unlabeled_indices), batch_size=batch_size, shuffle=False)
        ...
        aggregated_predictions = np.median(aligned_predictions, axis=0)
        probabilities = np.array([array[:, 1] for array in aggregated_predictions]).T
        ...
        valid_labeled_indices.extend(new_indices)
        labeled_indices = valid_labeled_indices
